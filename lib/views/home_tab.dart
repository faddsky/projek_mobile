          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text("Semangat pagi,", style: TextStyle(fontSize: 16, color: Colors.grey)),
                      Text("Dila ✨", 
                        style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.green[900])),
                    ],
                  ),
                  const Icon(Icons.notifications_none_rounded, size: 30, color: Color(0xFF6B8E23)),
                ],
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Text("Langkah Kakimu Hari Ini", style: TextStyle(color: Colors.white, fontSize: 16)),
                    ...
                    Obx(() => Text("🌱 Hemat ${controller.carbonSaved.value.toStringAsFixed(2)}g CO2", 
                    ...
                    ElevatedButton.icon(
                      onPressed: () async {
                        try {
                          await ApiService.sendEcoData("Aktivitas Berjalan", controller.stepCount.value);
                          Get.snackbar("Berhasil!", "Data tersimpan di Cloud Google ☁️",
                              snackPosition: SnackPosition.BOTTOM, 
                              backgroundColor: Colors.white.withOpacity(0.9));
                          setState(() {}); // Refresh list riwayat di bawah
                        } catch (e) {
                          Get.snackbar("Waduh!", "Server Cloud-nya belum nyala, Dil! 😅");
                        }
                      },
              ...
              // Bagian Tool Cards
              const Text("Eco AI Tools", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 15),
              Row(
                children: [
                  Expanded(
                    child: _buildToolCard("Scan Sampah", Icons.camera_enhance_rounded, Colors.orange[50]!, Colors.orange, () => controller.scanWaste()),
                  ),
                  const SizedBox(width: 15),
                  Expanded(
                    child: _buildToolCard("E-Waste Guide", Icons.auto_stories_rounded, Colors.blue[50]!, Colors.blue, () {}),
                  ),
                ],
              ),
              ...
              // RIWAYAT DARI CLOUD
              const Text("Riwayat Cloud Terbaru", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 15),
              FutureBuilder<List<dynamic>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    var items = snapshot.data!.reversed.toList(); // Terbaru di atas
